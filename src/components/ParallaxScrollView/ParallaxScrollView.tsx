...
import {
  ParallaxItemProps,
  ParallaxScrollViewContextType,
  ParallaxScrollViewProps,
} from "@/src";
import clsx from "clsx";

const ParallaxScrollViewContext =
  createContext<ParallaxScrollViewContextType | null>(null);
...
export const ParallaxScrollView = ({
  ...
  const index = React.Children.toArray(children).findIndex(
    (child) => isValidElement(child) && child.type === ParallaxItem,
  );

  if (index > 0)
    throw new Error(
      "<ParallaxItem /> needs to be the first component of the ScrollView!",
    );
...
export const ParallaxItem = ({ children, className }: ParallaxItemProps) => {
  ...
  const imageAnimatedStyle = useAnimatedStyle(() => {
    return {
      ...
      opacity: interpolate(scrollOffset.value, [-height, 0, height], [1, 1, 0]),
    };
  });
