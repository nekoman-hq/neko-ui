...
export const TextInput = ({
  ...
  const iconChild = useMemo(
    () =>
      React.Children.toArray(children).find(
        (child) => React.isValidElement(child) && child.type === TextInputIcon,
      ),
    [children],
  );
  const labelChild = useMemo(
    () =>
      React.Children.toArray(children).find(
        (child) => React.isValidElement(child) && child.type === TextInputLabel,
      ),
    [children],
  );
  const footerChild = useMemo(
    () =>
      React.Children.toArray(children).find(
        (child) =>
          React.isValidElement(child) && child.type === TextInputFooter,
      ),
    [children],
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          {labelChild !== undefined && labelChild}
        </Animated.View>
        ...
        >
          <ReactNativeTextInput
            ...
            placeholder={
              inputFocused ||
              (iconChild === undefined && labelChild === undefined)
                ? placeholder
                : undefined
            }
            selectTextOnFocus
...
export const TextInputIcon = ({
  ...
  return (
    <Animated.View style={animatedIconStyle}>
      <Icon className={clsx("color-foreground", className)} size={18} />
    </Animated.View>
  );
};
